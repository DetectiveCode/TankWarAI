...
Game::Game() // Constructor
{
	...
	//Create the tile grid for A* search
	//for (size_t i = 1; i < 39; i++)
	//{
	//	for (size_t k = 1; k < 28; k++)
	//	{
	//		tiles.push_back(Obstacle(i * 20, k * 20, i * 20 + 20, k * 20 + 20, sf::Color(40, 70, 20)));
	//	}
	//}
}
...
// Play the game for one timestep
void Game::play()
{
	// Move tank
	player.markPos();
...
void Game::draw(sf::RenderTarget &target, sf::RenderStates states) const// Draw the game
{
	...
	//Draw the A* star search tile grid
...
void Game::keyPressed(sf::Keyboard::Key key)
{
	...
	case sf::Keyboard::Tab:
		...
		for (list<Obstacle>::iterator it = redBuildings.begin(); it != redBuildings.end(); ++it) { it->toggleDebugMode(); }
		for (list<Obstacle>::iterator it = blueBuildings.begin(); it != blueBuildings.end(); ++it) { it->toggleDebugMode(); }
		//for (size_t i = 0; i < tiles.size(); i++) { tiles[i].toggleDebugMode(); } //Enable A* search grid debug mode
		break;
	case sf::Keyboard::W:
		player.goForward();
		break;
